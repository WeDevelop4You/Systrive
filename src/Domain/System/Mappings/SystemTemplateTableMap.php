<?php

namespace Domain\System\Mappings;

class SystemTemplateTableMap
{
	public const TABLE = 'system_templates';

	public const ID = 'id';
	public const VALUE = 'value';
	public const NAME = 'name';
	public const TYPE = 'type';
	public const IS_PUBLIC = 'is_public';
	public const CREATED_AT = 'created_at';
	public const UPDATED_AT = 'updated_at';

	public const TABLE_ID = 'system_templates.id';
	public const TABLE_VALUE = 'system_templates.value';
	public const TABLE_NAME = 'system_templates.name';
	public const TABLE_TYPE = 'system_templates.type';
	public const TABLE_IS_PUBLIC = 'system_templates.is_public';
	public const TABLE_CREATED_AT = 'system_templates.created_at';
	public const TABLE_UPDATED_AT = 'system_templates.updated_at';
}
